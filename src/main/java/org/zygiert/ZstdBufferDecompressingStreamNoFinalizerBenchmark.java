...
public class ZstdBufferDecompressingStreamNoFinalizerBenchmark {
    ...
    public static class ThroughputState {
        ...
        public void setup() throws IOException {
            try (InputStream is = getClass().getResourceAsStream("/dataset-formatted.xml")) {
                this.compressedData = ByteBuffer.wrap(Zstd.compress(is.readAllBytes(), LEVEL));
            }
            this.target = ByteBuffer.allocate(chunkSize);
        }
    }
    ...
    public int decompressionThroughput(ThroughputState state) throws IOException {
        ...
        try (ZstdBufferDecompressingStream in = new ZstdBufferDecompressingStream(state.compressedData.duplicate())) {
    ...
    public void createAndClose() throws IOException {
        try (ZstdBufferDecompressingStream in = new ZstdBufferDecompressingStream(ByteBuffer.allocate(0))) {
